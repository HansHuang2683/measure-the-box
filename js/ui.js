...
function _getAutoBoxPool() {
    ...
    const step = 5;
    const minSide = 30;
    const maxSide = CONFIG.maxSide || 60;
    for (let l = minSide; l <= maxSide; l += step) {
        for (let w = minSide; w <= maxSide; w += step) {
            for (let h = minSide; h <= maxSide; h += step) {
                ...
                const wall = CONFIG.defaultWallThickness;
                addBox({
                    id: 'auto_std_' + sorted.join('_'),
                    name: `推荐常规 ${sorted[0]}×${sorted[1]}×${sorted[2]}`,
                    external: dims(sorted[0], sorted[1], sorted[2]),
                    wallThickness: wall,
                    internal: dims(
                        Math.max(0.1, sorted[0] - wall * 2),
                        Math.max(0.1, sorted[1] - wall * 2),
                        Math.max(0.1, sorted[2] - wall * 2)
                    ),
                    autoGenerated: true,
                });
            }
...
function _ensureAutoBoxType(box) {
    ...
    const existing = boxTypes.find(b =>
        Math.abs(b.external.length - box.external.length) < 0.05 &&
        Math.abs(b.external.width - box.external.width) < 0.05 &&
        Math.abs(b.external.height - box.external.height) < 0.05 &&
...
function _scoreAutoFilledCandidate(cand, allSkus) {
    ...
    return allocatedRatio * 10000
        ...
        + hardRatio * 500
        - externalVol / 20000
        - Math.max(0, cand.n - 12) * 8
        - (cand.remainder || 0) * 12;
}

function _maximizeFillersForCandidate(baseCand, fillerSkus, allSkus) {
    ...
    let currentLayout = _validateAutoCandidate({ ...baseCand, assignments }, allSkus);
    ...
    while (true) {
        ...
        for (const sku of fillerSkus) {
            ...
            const testCand = { ...baseCand, assignments: testAssignments };
            const layout = _validateAutoCandidate(testCand, allSkus);
            if (!layout) continue;
...
function _pickBestAutoFilledCandidate(baseCandidates, fillerSkus, allSkus) {
    const sorted = [...baseCandidates].sort((a, b) => b.score - a.score).slice(0, 220);
    let best = null;
    for (const baseCand of sorted) {
        const filled = _maximizeFillersForCandidate(baseCand, fillerSkus, allSkus);
        if (!filled) continue;
...
function autoCreateGroups() {
    ...
    let bestMain = _pickBestAutoFilledCandidate(hardCandidates, skus, skus);
    if (!bestMain) bestMain = _validateAndPickBest(hardCandidates, skus);
    if (!bestMain) {
        alert('无法找到合适的装箱方案，请检查产品尺寸是否过大');
        return;
    }
    ...
    if (remainingAfterMain.length > 0) {
        ...
        const bestLeftover = _validateAndPickBest(leftoverCandidates, skus);
        if (bestLeftover) {
